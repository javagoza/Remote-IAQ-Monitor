...
*/
TFTSevenSegmentDecimalDisplay::TFTSevenSegmentDecimalDisplay(Adafruit_ST7735* tft,
    int16_t x = 0,
    int16_t y = 0,
    int16_t w = 16,
    int16_t h = 32,
...
void TFTSevenSegmentDecimalDisplay::display(int16_t number ) {
  display((number / 1000 ) % 10,(number / 100 ) % 10, (number / 10 ) % 10, number % 10);
}

/*!
   @brief    Display number
    @param   thousands number to display on the thousands subgroup module
    @param   hundreds number to display on the hundreds subgroup module
    @param   tens     number to display on the tens subgroup module
    @param   ones     number to display on the ones siubgroup module

*/
void TFTSevenSegmentDecimalDisplay::display(int16_t thousands, int16_t hundreds, int16_t tens, int16_t ones) {
